...
// Helper function to convert color names to CSS classes
...
export const TranscriptDetailPage: React.FC = () => {
  ...
  const transcriptTags = loadedTranscript?.tags?.length > 0 
    ? loadedTranscript.tags.map((tag, index) => ({
        name: tag.name,
        color: getTagColorClass(tag.color || 'gray') || getDefaultTagColor(index),
      }))
    : [
